...
vector<Block> find_blocks(vector<Point> & vp) {
  ...
  for (auto & p : vp) {
    divided_x.push_back(p.x);
    divided_x.push_back(p.x+1);
  }
  sort(divided_x.begin(), divided_x.end());
  divided_x.erase(unique(divided_x.begin(), divided_x.end()), divided_x.end());

  vector<tuple<Point, Point, int>> events;
  for (auto & line : top_bottom) {
    Point a, b;
    int sign;
    tie(a, b, sign) = line;
    Point pdir = b.y > a.y ? Point(1, 1) : Point(1, 0);
    auto it = lower_bound(divided_x.begin(), divided_x.end(), a.x);
    while (a.x <= b.x) {
      it = next(it);
      assert(*it <= b.x+1);
      events.emplace_back(a, a + pdir * (*it-a.x-1), sign);
      a = a + pdir * (*it - a.x);
    }
  }
  ...
  for (int i = 0; i + 1 < (int)events.size(); ++i) {
    if (i & 1) {
      assert(get<0>(events[i]) < get<0>(events[i+1]));
    } else {
      assert(get<2>(events[i]) < 0 && get<2>(events[i+1]) > 0);
      assert(get<0>(events[i]).x == get<0>(events[i+1]).x);
      assert(get<1>(events[i]).x == get<1>(events[i+1]).x);
      result.emplace_back(
          Column(get<0>(events[i]).x, get<0>(events[i]).y+1,
                 get<0>(events[i+1]).y),
          Column(get<1>(events[i]).x, get<1>(events[i]).y+1,
                 get<1>(events[i+1]).y)
      );
    }
  }
