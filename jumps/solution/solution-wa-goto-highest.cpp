// WA solution: This solution may still go to the right even if already in
// [C, D]. It will stop at the highest point in [C, D] instead.
...
int minimum_jumps(int A, int B, int C, int D) {
  ...
  int s = B;
  for (int j = kMaxLogN - 1; j >= 0; --j) {
    if (A <= L[j][s] && H[L[j][s]] < H[middle_tallest_index]) {
      s = L[j][s];
    }
  }
  int jumps = 0;
